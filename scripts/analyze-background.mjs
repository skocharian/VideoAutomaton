...
const metadata = probeSource(args.input, ffprobePath);
const artifact = await analyzeBackground({
  input: args.input,
...
async function analyzeBackground({ input, assetKey, metadata, ffmpegPath }) {
  const sizes = {};
  const analysisSizes = buildAnalysisSizes(renderConfig.sizes);
  const frameCount = metadata.isStillImage ? 1 : Math.max(12, renderConfig.sampleRatios.length * 3);
  const sampleTimes = buildSampleTimes(metadata.duration, frameCount);

  for (const [size, config] of Object.entries(renderConfig.sizes)) {
    ...
    for (const sampleTime of sampleTimes) {
      ...
      frames.push({
        ...
        regions: analyzeRegions(rgba, scaledCrop, size, analysisSizes[size]),
      });
    }
...
function probeSource(input, ffprobePath) {
...
function analyzeRegions(frame, crop, size, analysisSize) {
  const regionLayouts = getRegionLayouts(size);
...
function getRegionLayouts(size) {
...
function buildSampleTimes(duration, frameCount) {
  if (duration <= 1.01) {
    return [0];
  }
  ...
  for (const ratio of renderConfig.sampleRatios) {
    times.add(Number((Math.min(duration * ratio, maxTime)).toFixed(3)));
  }
